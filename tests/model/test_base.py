...
def test_init_explicit_key_wins_over_env(monkeypatch):
    monkeypatch.setenv("OPENROUTER_API_KEY", "env-key")
    model = Model(api_key="explicit-key")
    assert model._api_key == "explicit-key"
...
def test_assert_open_weight_accepts_open_weight_model(monkeypatch):
    _patch_catalog(monkeypatch, {"open/model": {"hugging_face_id": "org/open-model"}})
    model = Model(model_id="open/model", api_key="k")
    # Should not raise.
    model._assert_open_weight()


def test_assert_open_weight_rejects_unknown_model(monkeypatch):
    _patch_catalog(monkeypatch, {"open/model": {"hugging_face_id": "org/x"}})
...
def test_assert_open_weight_rejects_closed_weight_model(monkeypatch, info):
    _patch_catalog(monkeypatch, {"closed/model": info})
    model = Model(model_id="closed/model", api_key="k")
    with pytest.raises(ValueError, match="no hugging_face_id"):
        model._assert_open_weight()


def test_ensure_open_weight_checks_once_then_latches(monkeypatch):
...
def test_ensure_open_weight_skipped_when_not_enforced(monkeypatch):
    model = Model(api_key="k")
...
@pytest.mark.parametrize("method", ["assert_structured_output", "assert_generation"])
def test_assert_helpers_pass_through_nonempty(method):
    model = Model(api_key="k")
...
def test_assert_helpers_raise_on_empty(method, empty):
    model = Model(api_key="k")
...
@pytest.mark.parametrize("method", ["assert_structured_output", "assert_generation"])
def test_assert_helpers_sample_id_in_message(method):
    model = Model(api_key="k")
...
@pytest.mark.parametrize("method", ["assert_structured_output", "assert_generation"])
def test_assert_helpers_no_sample_id_context_when_absent(method):
...
def test_assert_helpers_message_specific_to_method(method, phrase):
...
def test_instruction_base_default_is_empty():
    assert Model(api_key="k").get_instruction() == ""


def test_instruction_custom_overrides_default():
    model = Model(api_key="k")
    model.set_instruction("be terse")
    assert model.get_instruction() == "be terse"


def test_instruction_subclass_default_used_until_overridden():
    class Styled(Model):
...
def test_build_llm_constructs_openai_llm_with_expected_kwargs(monkeypatch):
...
def test_build_llm_default_structured_output_is_none(monkeypatch):
    monkeypatch.setattr(base_mod, "OpenAILLM", _RecordingLLM)
    model = Model(api_key="k")
...
def test_run_pipeline_wires_and_runs(monkeypatch):
    events = []
