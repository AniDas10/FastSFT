...
class TestScoreSamples:
    def test_maps_ids_to_scores(self, monkeypatch, fake_distiset):
        captured = {}
    ...
    def test_uses_prompt_override(self, monkeypatch, fake_distiset):
        captured = {}
    ...
    def test_raises_on_empty_generation(self, monkeypatch, fake_distiset):
        judge = Judge(api_key="k")
        monkeypatch.setattr(
            judge,
            "run_pipeline",
...
class TestFailedSampleCount:
    ...
    def test_counts_below_threshold(self, scores, threshold, expected):
...
class TestComparisons:
    def _verdict_run(self, fake_distiset, mapping):
    ...
    def test_compare_samples_maps_ids_to_verdicts(self, monkeypatch, fake_distiset):
        judge = Judge(api_key="k")
        monkeypatch.setattr(
            judge, "run_pipeline", self._verdict_run(fake_distiset, {"1": "A", "2": "tie"})
        )
    ...
    def test_compare_to_reference_maps_ids_to_verdicts(self, monkeypatch, fake_distiset):
        judge = Judge(api_key="k")
        monkeypatch.setattr(
            judge, "run_pipeline", self._verdict_run(fake_distiset, {"1": "B"})
        )
        result = judge.compare_to_reference({"1": ("q", "ref", "ans a", "ans b")})
        assert result == {"1": Verdict(winner="B")}

    def test_comparison_prompt_renders_question_and_both_answers(self):
    ...
    def test_reference_prompt_renders_reference_and_both_answers(self):
