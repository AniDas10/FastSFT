"""CLI entry point for the evaluation module:
    ...
    uv run python -m eval.run [modelsets/<run_id>]
...
to the adapter. View the result with `python -m eval.results_viewer`.
"""
...
from constants import DEFAULT_JUDGE_MODEL, DEFAULT_PARENT_MODEL, MODELSETS_OUTPUT_DIR
from data.constants import DEFAULT_PARENT_TEMPERATURE
...
from eval.prompt_set import EvalPromptSet
from eval.results import save_results
from helper import (
    current_timestamp,
    latest_run_path,
...
from validation_checks import validate_eval_flags
...
def main():
    ...
    log(
        ...
        "View with: uv run python -m eval.results_viewer"
    )
